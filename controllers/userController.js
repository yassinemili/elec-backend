...
const createUser = async (req, res) => {
    try {
        ...
        const user = new User({ name: name, passwordHash: hashedPassword, role: role, teamId: teamId });
        await user.save();
...
const updateUserRole = async (req, res) => {
    try {
        const { role } = req.body;
        if (!role) return res.status(400).json({ message: "Missing required field" });
...
const getUserSubmissions = async (req, res) => {
    try {
        const submissions = await Submission.find({ userId: req.params.id }).populate("challengeId", "title");
        if (!submissions) return res.status(404).json({ message: "User not found" });
...
const getUserParticipatedCompetitions = async (req, res) => {
    try {
        const competitions = await Competition.find({ teams: { $in: [req.params.id] } });
        if (!competitions) return res.status(404).json({ message: "User not found" });
...
const getUserParticipatedChallenges = async (req, res) => {
    try {
        const challenges = await Challenge.find({ users: { $in: [req.params.id] } });
